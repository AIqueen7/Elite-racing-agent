...
with torch.no_grad():
    ...
    # Simulated 100Hz heat history (High-load braking events)
...
with tabs[0]:
    ...
    with col2:
        ...
        st.write("""
        **Global Maxima ($O^*$):** This star represents the 'Golden Window.' It’s the mathematical coordinate where your engine's torque delivery and chassis weight are in perfect phase.
        
        **Why this wows:** After 40 years of tuning by 'feel,' this manifold identifies the exact setup where the car 'sings,' proving if your aero is supporting or fighting your mechanical grip.
        """)
...
with tabs[1]:
    ...
    ax1.axhline(carcass_forecast, color='red', ls='--', label=f"Carcass Memory: {carcass_forecast:.1f}°C")
    ax1.set_ylabel("Temp (°C)"); ax1.legend(); st.pyplot(fig1); plt.close(fig1)
    st.write(f"**Thermal Intelligence:** With **{hp}HP**, the carcass is under extreme stress. The LSTM remembers the last 3 braking zones to calculate the internal heat soak that sensors miss.")
...
with tabs[2]:
    ...
    st.warning(f"**Resonance Warning:** {mat} uprights vibrate at {hz}Hz. Set dampers to negate this node.")
...
with tabs[3]:
    ...
    st.info(f"Optimized for: {hp}BHP | {kg}kg | {mat} Construction.")
